...
pub(super) async fn gather(
    ...
    // stack runs on Tautulli + Maintainerr + Sonarr/Radarr alone. Set both
    // Settings (the UI) first, then FLINCH_PLEX_URL / FLINCH_PLEX_TOKEN, to
    // add item-level state on top.
    let pick = |configured: &str, env: &str| {
        if configured.trim().is_empty() { std::env::var(env).unwrap_or_default() } else { configured.to_string() }
    };
    let (plex_url, plex_token) = (with_scheme(&pick(&settings.plex_url, "FLINCH_PLEX_URL")), pick(&settings.plex_token, "FLINCH_PLEX_TOKEN"));

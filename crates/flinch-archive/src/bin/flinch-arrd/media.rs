...
impl PlexClient<'_> {
    async fn get(&self, path: &str) -> anyhow::Result<PlexContainer> {
        // Token in BOTH places: some Plex endpoints answer an XML error to a
        // header-only token, which then fails JSON parsing with a confusing
        // "unexpected shape".
        let separator = if path.contains('?') { '&' } else { '?' };
        let response = self
            .http
            .get(format!("{}{path}{separator}X-Plex-Token={}", self.base, self.token))
            .header("X-Plex-Token", self.token)
            .header("Accept", "application/json")
            .send()
            .await
            // The URL carries the token: keep it out of every error.
            .map_err(reqwest::Error::without_url)
            .context("plex request failed")?;
        let status = response.status();
        let body = response.text().await.map_err(reqwest::Error::without_url).context("plex body read failed")?;
    ...
    async fn keep_keys(&self, section: &str, tag: &str) -> anyhow::Result<Vec<String>> {
        ...
        for label in labels.directory.iter().filter(|label| named(&label.title) && !label.fast_key.is_empty()) {
            keys.extend(self.listing(&label.fast_key).await?.into_iter().map(|row| row.rating_key));
        }
...
async fn fetch_keep_history(http: &reqwest::Client, base: &str, key: &str) -> anyhow::Result<tautulli::KeepHistory> {
    let get = |command: &'static str| async move {
        ...
        // The URL carries the API key: keep it out of every error.
        let response = http.get(&url).send().await.and_then(reqwest::Response::error_for_status);
        response.map_err(reqwest::Error::without_url)?.text().await.map_err(reqwest::Error::without_url)
    };

...
impl EvictionLedger {
    ...
    fn settle(&mut self, volume: &str, occupancy: Occupancy, recycle_secs: &impl Fn(App) -> u64, now: u64) {
        ...
        // Sizes come from the *arrs: an absurd one must saturate, never wrap
        // into a drop that frees credit (or panic a debug build).
        let other = signed(occupancy.used).saturating_sub(signed(occupancy.library)).saturating_sub(signed(credited));
        if gone.is_empty() {
        ...
        for (due_at, gone_at, id) in gone.into_iter().filter(|(due_at, _, _)| now >= *due_at) {
            ...
            let dropped = watch.baseline.saturating_sub(watch.low).saturating_mul(2) >= bytes;
            if unjudgeable || dropped {
                if !unjudgeable {
                    watch.low = watch.low.saturating_add(bytes);
                }
        ...
        self.last_other.insert(volume.to_string(), other.saturating_add(released));
    }

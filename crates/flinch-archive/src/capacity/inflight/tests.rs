...
#[test]
fn absurd_sizes_saturate_and_never_read_as_space_freed() {
    // A broken *arr can report any size. Wrapping arithmetic once turned such
    // a reading into a huge "drop" that released the credit, or panicked.
    let mut ledger = EvictionLedger::default();
    ledger.record(HandedOver { id: "radarr-1", title: "radarr-1", app: App::Radarr, volume: "/media", bytes: u64::MAX }, 0);
    let absurd = BTreeMap::from([("/media".to_string(), Occupancy { used: 0, library: u64::MAX })]);
    ledger.observe(|_| true, three_days, &absurd, DAY);
    ledger.observe(|_| false, three_days, &absurd, 2 * DAY);
    ledger.observe(|_| false, three_days, &absurd, 5 * DAY);
    assert_eq!(credit(&ledger).pending, u64::MAX, "no measured drop, so the eviction is still credited");
}

...
/// `grace_runs` consecutive appearances — in `candidates` order, which is the
/// plan's eviction order: the caps meet the least-regret items first, and an
/// item deferred by a cap keeps its place for the next run.
pub fn advance_streaks(state: &mut CandidateState, candidates: &[String], grace_runs: u32, now: u64) -> Vec<String> {
    ...
    candidates
        .iter()
        .filter(|id| state.streaks.get(*id).is_some_and(|streak| *streak >= grace_runs.max(1)))
        .cloned()
        .collect()
}

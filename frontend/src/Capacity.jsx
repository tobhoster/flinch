 ...
 * free but bytes still in a recycle bin is waiting, not done.
 */
function phaseOf(v) {
  if (!v.latched) return 'idle';
  if (v.covered === false) return 'short';
  if (v.goal_bytes === 0 && (v.pending_bytes || 0) > 0) return 'waiting';
  return 'freeing';
}
...
function VolumeRow({ v, ceiling, release }) {
  ...
  const state = {
    ...
    waiting: `waiting on recycle bin, ${GiB(pending)} GiB`,
